...
export function ThemeToggle({ preference, onChange, variant = 'segmented' }: ThemeToggleProps) {
  if (variant === 'compact') {
    ...
    return (
      <button
        ...
        aria-label={`${label} theme. Click to change.`}><i className={`ti ${icon}`} aria-hidden="true" /></button>
    );
  }
    ...
    <div className="theme-toggle" role="group" aria-label="Color theme">
      {OPTIONS.map((option) => (
        <button
          ...
          aria-pressed={preference === option.value}><i className={`ti ${option.icon}`} aria-hidden="true" /></button>
      ))}
    </div>
  );
}
