  ...
  async function handleBuild() {
    ...
    for (let i = 0; i < allNames.length; i++) {
      ...
      try {
        ...
        setBuildLog((l) =>
          l.map((row, idx) =>
            idx === l.length - 1 ? { text: `${name} researched`, status: 'ok' } : row,
          ),
        );
      } catch (e) {
    ...
    setBuildLog((l) => [
      ...l,
      { text: 'Synthesizing differentiation opportunities…', status: 'pending' },
    ]);
    try {
      opportunities = await researchOpportunities(anchorTrimmed, brands);
      setBuildLog((l) =>
        l.map((row, idx) =>
          idx === l.length - 1 ? { text: 'Opportunities synthesized', status: 'ok' } : row,
        ),
      );
    } catch (e) {
      const message = e instanceof Error ? e.message : 'Unknown error';
      setBuildLog((l) =>
        l.map((row, idx) =>
          idx === l.length - 1
            ? { text: `Opportunity synthesis failed (${message})`, status: 'err' }
            : row,
        ),
      );
    }
    ...
    try {
      ...
      setBuildLog((l) =>
        l.map((row, idx) =>
          idx === l.length - 1 ? { text: 'Cross-brand themes identified', status: 'ok' } : row,
        ),
      );
    } catch (e) {
      const message = e instanceof Error ? e.message : 'Unknown error';
      setBuildLog((l) =>
        l.map((row, idx) =>
          idx === l.length - 1
            ? { text: `Cross-brand theme analysis failed (${message})`, status: 'err' }
            : row,
        ),
      );
    }
  ...
  return (
    <div className="setup-wrap">
      ...
      <div className="setup-sub">
        ...
        one&apos;s positioning, content themes, tone, and channel scale, then generate a live
        dashboard you can keep refreshing.
      </div>
